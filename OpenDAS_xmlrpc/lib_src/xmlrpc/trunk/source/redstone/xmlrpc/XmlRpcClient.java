...
public class XmlRpcClient extends XmlRpcParser implements XmlRpcInvocationHandler
{
            ...
            new Thread()
            {
                public void run()
                {
                    ...
                    catch ( XmlRpcFault e )
                    {
                        ...
                        try
                        {
                        	callback.onFault( 42,fault.getString( "faultString" )+"\n"+fault.getString( "faultCode" ));
                        }
                        catch ( Exception ee )
                        {
    ...
    private void handleResponse() throws XmlRpcFault
    {
        ...
        if ( isFaultResponse )
        {
            ...
            try
            {
            	f = new XmlRpcFault( 42,fault.getString( "faultString" )+"\n"+fault.getString( "faultCode" ) );
            }
            catch ( Exception e )
            {
            	f = new XmlRpcFault( fault.getInteger("faultCode" ),fault.getString( "faultString" ) );
            }
            throw f; 
        }
    }
